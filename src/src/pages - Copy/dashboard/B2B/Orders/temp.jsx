...
const NewOrder = () => {
  ...
  const [allOrderProducts, orderProducts] = useLazyQuery(GET_ALL_PRODUCTS_PARENT_ID, {
     variables:{
      "first": 100,
      "filter": {
      "attributes": [
  ...
  "sort": {
    ...
    onCompleted: (data) => {
      console.log(data);
      setLocalData(data.products.edges)
    },
    onError: (err) => {
      console.log(err);
     }
  ...
  const handleSearch = (e) => {
    ...
    if (val !== "" || val !== undefined)
      setLocalData(
        orderProducts?.data?.products?.edges?.filter(
          (edge) =>
            edge?.node?.name?.toLowerCase()?.includes(val) ||
            edge?.node?.variants[0]?.name?.toLowerCase()?.includes(val)
        )
      );
    else setLocalData(orderProducts?.data?.product?.edges);
  };
  const columns = [
    {
      title: "Product",
        ...
        <div className='fixed top-0 left-0 h-full w-full flex items-center justify-center bg-black/50 backdrop-blur-sm z-[99999]'>
          <div className='bg-white w-10/12 md:w-4/5 h-3/4 overflow-scroll flex flex-col justify-between rounded-md'>
            <div className='p-2 h-full overflow-y-scroll dark:text-slate-200 dark:bg-slate-800'>
              ...
              <div className="flex">
              <section className='h-full flex flex-wrap gap-2 items-start justify-start w-1/2  p-2 overflow-y-scroll '>
                Parent Products <br />
                {localData?.map((prod) => (
                  <div
                    key={prod?.node?.id}
                    ...
                    allOrderProducts({ variables:{
                      ...
                      onCompleted: (data) => {
                        console.log(data);
                      setVariantData(data.products.edges)
                    }})
                      // setOpen(false);
                    }}
                  >
                    ...
                    <div className='flex flex-col gap-2'>
                      ...
                      <span className='text-xs'>
                        {prod?.node?.variants?.length>0? prod?.node?.variants[0]?.name : "no sku available"}
                      </span>
              ...
              <section className='h-full flex flex-wrap gap-2 items-start justify-start w-1/2  p-2 overflow-y-scroll '>
                  Variants {orderProducts?.loading && <Spinner />}
